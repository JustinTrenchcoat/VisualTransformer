...
SUBMISSION_DF_DIR = "dataset/sample_sumbission.csv"
...
model = Vit(IN_CHANNELS,BATCH_SIZE,EMBED_DIM,NUM_PATCHES,DROPOUT,
            NUM_HEADS,ACTIVATION,NUM_ENCODER,NUM_CLASSES)
...
for epoch in EPOCHS:
    ...
    for idx, img_label in enumerate(tqdm(train_dataloader,position=0,leave=True)):
        ...
        label = img.label["label"].type(torch.uint8).to(device)
        y_pred = model(img)
